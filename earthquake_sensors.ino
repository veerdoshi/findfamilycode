...
void setup() {
 ...
 // lcd.clear();
  ...
  if(sStatus == 1) { // FONA is on
    ...
    delay(3000); // Give FONA time to register 
    // Check registration
    fonaReg();
 ...
 */
  ...
  { // FONA not registered on network
    Serial.println(F("FONA not reg., End routine"));
    delay(2000); 
    quakechecker();      
  }
/*  
  Serial.println(F("Pause 1m"));
  delay(6000);
*/
}
...
void quakechecker() {
  ...
  if (earthquakesign < (earthquakestart-7)) {
/*
    lcd.clear();
    lcd.print("|||| ALERT ||||");
    lcd.setCursor(0,1);
